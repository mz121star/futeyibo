define(['jquery-ui', 'jquery-fullPage', 'jquery-mousewheel','coveringBad','head','jquery.easing.1.3','modernizr'], function () {
    ...
    $('#fullpage').fullpage({
        ...
        afterLoad: function(anchorLink, index, slideAnchor, slideIndex){
            $('.tip').show();
            if(index==1){
                    $(".bg4-1").delay(100).animate({'opacity': 1,top:'220px',right:'245px'},1000);
                }
            if(index==3){
            ...
            if(!Modernizr.csstransitions){
                ...
                if(index==3){
                     $(".bg4-31").delay(1000).animate({opacity:0},4000);
        ...
        onLeave: function(index, nextIndex, direction){
            $('.tip').hide();
            if(index==1){
                    $(".bg4-1").animate({'opacity': 0,top:'2000px',right:'245px'});
                }
            if(index==6){
                hasShow = false;
                $('.bg4-61').animate({'top':'200px'},0);
            ...
            if(!Modernizr.csstransitions){
                ...
                if(index==3){
                    $(".bg4-31").delay(1000).animate({opacity:1},1000);
...
*/
    ...
    $('.standard').on('fitbg',function(){
        $(this).css('margin-left',-($(this).width() - $("body").width())/2);
    });
